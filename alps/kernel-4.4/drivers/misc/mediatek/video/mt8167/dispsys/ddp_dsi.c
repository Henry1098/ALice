	...
	*
	...
	for (i = 0; i < sizeof(struct DSI_CMDQ_REGS); i += 16) {
		DDPMSG("DSI_CMD+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i,
			       INREG32((DDP_REG_BASE_DSI0 + 0x180 + i)),
			       INREG32((DDP_REG_BASE_DSI0 + 0x180 + i + 0x4)),
			       INREG32((DDP_REG_BASE_DSI0 + 0x180 + i + 0x8)),
			       INREG32((DDP_REG_BASE_DSI0 + 0x180 + i + 0xc)));
	}
...
int ddp_dsi_build_cmdq(enum DISP_MODULE_ENUM module, void *cmdq_trigger_handle, enum CMDQ_STATE state)
{
	...
	//bug 356116,yangzheng.wt,20180521,add for multi register ESD check
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		for (i = 0; i < 3; i++) {
			...
			DISPDBG("esd [DSI]enter cmp read_data0 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x\n",
				read_data0.byte0,
				read_data0.byte1,
				read_data0.byte2,
				read_data0.byte3);
			DISPDBG
			    ("esd [DSI]enter cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x\n",
			...
			if (packet_type == 0x1A || packet_type == 0x1C) {
				...
				if (recv_data_cnt > dsi_params->lcm_esd_check_table[i].count) {
					...
					DISPCHECK
					("esd Set receive data count from %d to %d as ESD check table specified.\n",
						recv_data_cnt, dsi_params->lcm_esd_check_table[i].count);
					recv_data_cnt = dsi_params->lcm_esd_check_table[i].count;
				}
				DISPCHECK("esd DSI read short packet size: %d\n", recv_data_cnt);
