...
#define JUST_INPUT_NO_SWITCH  1  //bug 356277 chaofubang.wt added headset node. 20180524
...
static int key_check(int b)
{
	...
	//+bug 341071 chaofubang.wt modified to fix button issue. 20180211
	if ((b < down_key_four) && (b >= up_key_four))
		return DW_KEY;
	else if ((b < up_key_four) && (b >= voice_key_four))
		return UP_KEY;
	else if ((b < voice_key_four) && (b >= 0))
		return MD_KEY;
	//-bug 341071 chaofubang.wt modified to fix button issue. 20180211
...
#ifdef ACCDET_MULTI_KEY_FEATURE
			ACCDET_DEBUG("[Accdet]send plug out in micbias\n");
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (eint_accdet_sync_flag == 1)
				accdet_status = PLUG_OUT;
			else
...
#endif
#endif
			headset_plug_out(); //Extb AQUA 4734 4737 chaofubang.wt added to solve headset icon not disappeared when plugged out. 20180620
			} else {
