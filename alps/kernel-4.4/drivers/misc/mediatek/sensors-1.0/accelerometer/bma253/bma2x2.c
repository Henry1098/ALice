...
#define GSENSOR_CONFIG_PROC_FILE    "G-sensor-name"
/*----------------------------------------------------------------------------*/
#define DEBUG 1
/*----------------------------------------------------------------------------*/
...
static struct proc_dir_entry *bma253_config_proc;
/*tad3sgh add --*/
...
static int BMA2x2_remap_to_gravity(struct i2c_client *client, int data[BMA2x2_AXES_NUM]);
//wt for acc cali end
...
#if !defined(CONFIG_HAS_EARLYSUSPEND)
static int bma2x2_suspend(struct device *dev,pm_message_t state);
...
static ssize_t bma253_config_write_proc(struct file *file, const char *buffer, size_t count,
					loff_t *ppos)
{
        return 0;
}
static ssize_t bma253_config_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
    int err = -1 ;
    char name[20] = "BMA253";
  
     err = copy_to_user(buffer, name, strlen(name)+1);
     if(err)
        printk("kxtj2 copy to user error\n");

    return 0;
}
static const struct file_operations bma253_upgrade_proc_fops = {
	.write = bma253_config_write_proc,
	.read = bma253_config_read_proc,
};
/*zwk.wt 20180308,add gsensor mmigroup hardwareinfomation start*/
/*----------------------------------------------------------------------------*/
...
{
	struct bma2x2_i2c_data *obj = i2c_get_clientdata(client);
	int acc_tmp[BMA2x2_AXES_NUM]={0};
	if(NULL == client)
	{
		return -EINVAL;
	}
	acc_tmp[obj->cvt.map[BMA2x2_AXIS_X]] = obj->cvt.sign[BMA2x2_AXIS_X]* data[BMA2x2_AXIS_X];
	acc_tmp[obj->cvt.map[BMA2x2_AXIS_Y]] = obj->cvt.sign[BMA2x2_AXIS_Y]* data[BMA2x2_AXIS_Y];
	acc_tmp[obj->cvt.map[BMA2x2_AXIS_Z]] = obj->cvt.sign[BMA2x2_AXIS_Z]* data[BMA2x2_AXIS_Z];
	data[BMA2x2_AXIS_X] = acc_tmp[BMA2x2_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[BMA2x2_AXIS_Y] = acc_tmp[BMA2x2_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[BMA2x2_AXIS_Z] = acc_tmp[BMA2x2_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	return 0;
...
#ifdef CONFIG_COMPAT
static long bma2x2_compat_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
    long err = 0;
	...
	case COMPAT_GSENSOR_IOCTL_INIT:
		...
		if (err){
			GSE_ERR("bma2x2 COMPAT_GSENSOR_IOCTL_INIT  failed.");
			return err;
		}
		break;
	...
	case COMPAT_GSENSOR_IOCTL_READ_CHIPINFO:
		    ...
		    if (err){
		        GSE_ERR("bma2x2 COMPAT_GSENSOR_IOCTL_READ_CHIPINFO  failed.");
		        return err;
		    }				 
		break;	  
	...
	case COMPAT_GSENSOR_IOCTL_READ_SENSORDATA:
		    ...
		    if (err){
		        GSE_ERR("bma2x2 COMPAT_GSENSOR_IOCTL_READ_SENSORDATA  failed.");
		        return err;
		    }
	...
	case COMPAT_GSENSOR_IOCTL_READ_GAIN:
		    ...
		    if (err){
		        GSE_ERR("bma2x2 COMPAT_GSENSOR_IOCTL_READ_GAIN  failed.");
		        return err;
		    }			 
		break;
	...
	case COMPAT_GSENSOR_IOCTL_READ_RAW_DATA:
		...
		if (err){
			GSE_ERR("bma2x2 COMPAT_GSENSOR_IOCTL_READ_RAW_DATA  failed.");
			return err;
		}
	...
	case COMPAT_GSENSOR_IOCTL_SET_CALI:
		...
		if (err){
			GSE_ERR("bma2x2 COMPAT_GSENSOR_IOCTL_SET_CALI  failed.");
			return err;
		}
	...
	case COMPAT_GSENSOR_IOCTL_CLR_CALI:
		...
		if (err){
			GSE_ERR("bma2x2 COMPAT_GSENSOR_IOCTL_CLR_CALI  failed.");
			return err;
		}
		break;
	...
	case COMPAT_GSENSOR_IOCTL_GET_CALI:
		    ...
		    if (err){
		        GSE_ERR("bma2x2 COMPAT_GSENSOR_IOCTL_GET_CALI  failed.");
		        return err;
		    }		
		break;
	...
	/*************************************************/
        ...
        default:
            GSE_ERR("bma2x2 unknown IOCTL: 0x%08x\n", cmd);
            err = -ENOIOCTLCMD;
        break;
	 ...
	 unsigned int cmd, unsigned long arg)
{
	...
	int cali[3];
	//wt for acc cali start
    s16	raw_data[BMA2x2_AXES_NUM+1] = {0};
	int acc_tmp[BMA2x2_AXES_NUM] = {0};
    //wt for acc cali end
	...
	case GSENSOR_IOCTL_READ_SENSORDATA:
		...
		if (data == NULL) {
           ...
           if((err = BMA2x2_ReadData(client, raw_data))!=0)
           {		 
           ...
           else
           {
               ...
               err = BMA2x2_remap_to_gravity(client, acc_tmp);
               if(err){
                   err = -EFAULT;
                   break;
               }
  			...
  			sprintf(strbuf, "%04x %04x %04x", sensor_data.x, sensor_data.y, sensor_data.z);
		   if(copy_to_user(data, strbuf, strlen(strbuf)+1))
		   {
			   err = -EFAULT;
			   break;	 
	...
	*/
	...
	case GSENSOR_IOCTL_SET_CALI:
		...
		if (atomic_read(&obj->suspend)) {
			...
			obj->cali_sw[BMA2x2_AXIS_X] = sensor_data.x * obj->reso->sensitivity / GRAVITY_EARTH_1000;
			obj->cali_sw[BMA2x2_AXIS_Y] = sensor_data.y * obj->reso->sensitivity / GRAVITY_EARTH_1000;
			obj->cali_sw[BMA2x2_AXIS_Z]= sensor_data.z * obj->reso->sensitivity / GRAVITY_EARTH_1000;	

			obj->cali_sw[BMA2x2_AXIS_X]=obj->cali_sw[BMA2x2_AXIS_X]*obj->cvt.sign[BMA2x2_AXIS_X];
			obj->cali_sw[BMA2x2_AXIS_Y]=obj->cali_sw[BMA2x2_AXIS_Y]*obj->cvt.sign[BMA2x2_AXIS_Y];
			obj->cali_sw[BMA2x2_AXIS_Z]=obj->cali_sw[BMA2x2_AXIS_Z]*obj->cvt.sign[BMA2x2_AXIS_Z];
...
static const struct file_operations bma2x2_fops = {
	...
	#ifdef CONFIG_COMPAT
	.compat_ioctl = bma2x2_compat_ioctl,
	#endif
};
/*----------------------------------------------------------------------------*/
	 ...
	 const struct i2c_device_id *id)
{
	...
	if (err<0) {
		GSE_ERR("bma2x2 get cust_baro dts info fail\n");
		goto exit;
	}
...
#endif
     bma253_config_proc = proc_create(GSENSOR_CONFIG_PROC_FILE, 0660, NULL, &bma253_upgrade_proc_fops);
     if( bma253_config_proc == NULL)
            GSE_ERR("%s: bma253 create proc node fail \n", __func__);
	bma2x2_init_flag = 0;
	GSE_LOG("%s: OK\n", __func__);
	return 0;
