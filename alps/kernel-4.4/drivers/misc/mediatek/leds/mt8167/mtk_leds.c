...
//+Bug-340219,guojunbo.wt,ADD,20180207,for add charging led control node
struct pinctrl *ledctrl = NULL; 
struct pinctrl_state *led_default = NULL;
struct pinctrl_state *led_on = NULL;
struct pinctrl_state *led_off = NULL;
//-Bug-340219,guojunbo.wt,ADD,20180207,for add charging led control node
...
//+Bug-340219,guojunbo.wt,ADD,20180207,for add charging led control node
void gpio_set_leds_level(unsigned int level)
{
        printk("gpio_set_leds_level=%d",level);
	if (level == 0)
	   pinctrl_select_state(ledctrl, led_off);
	else
	   pinctrl_select_state(ledctrl, led_on);
...
#ifdef CONFIG_MTK_AAL_SUPPORT
	if ((led_data->level != level) || (strcmp(led_data->cust.name, "red") == 0)){ //Bug-340219,guojunbo.wt,ADD,20180207,for add charging led control node
...
static int red_led_probe(struct platform_device *pdev)
{
    ...
    if (IS_ERR(ledctrl)) {
        LEDS_DEBUG("cannot find ledctrl\n");
     }

    led_default = pinctrl_lookup_state(ledctrl, "default");
    if (IS_ERR(led_default)) {
        LEDS_DEBUG("cannot find ledctrl led_default\n");
    }
    led_on = pinctrl_lookup_state(ledctrl, "led_on");
    if (IS_ERR(led_default)) {
        LEDS_DEBUG("cannot find ledctrl led_on\n");
    }
    led_off = pinctrl_lookup_state(ledctrl, "led_off");
    if (IS_ERR(led_default)) {
    LEDS_DEBUG("cannot find ledctrl led_off\n");
    }
...
MODULE_DEVICE_TABLE(of, red_led_of_match);
#endif
...
static int __init red_led_init(void)
{
	int ret;

	printk("red_led_init\n");

	ret = platform_driver_register(&red_led_driver);
	if (ret) {
...
static void __exit red_led_exit(void)
{
}
module_init(red_led_init);
module_exit(red_led_exit);
//-Bug-340219,guojunbo.wt,ADD,20180207,for add charging led control node
