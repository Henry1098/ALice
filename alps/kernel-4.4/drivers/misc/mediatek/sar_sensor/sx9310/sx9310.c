...
static int of_get_sx9310_platform_data(struct device *dev)
{
	...
	node = of_find_compatible_node(NULL, NULL, "sx9310");
	if (node) {
...
// fanhonggen check i2c  end 20160308
	...
	//	this->get_nirq_low = pplatData->get_is_nirq_low;
		...
		sx93XX_init(this);
		//bug 274358,zhangyonggang.wt,modify,20170705,add sar sensor hardware_info
		//hardwareinfo_set_prop(HARDWARE_SAR,"sx9310");
		
		read_register(this,0x03,&data);
		printk("%s read  03reg: %d\n",__FUNCTION__,data);
		read_register(this,0x42,&data);
		printk("%s read  42reg: %d\n",__FUNCTION__,data);
