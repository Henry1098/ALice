...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC2375HMIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h

    .checksum_value = 0xf7375923,        //checksum value for Camera Auto Test

    .pre = {
        .pclk = 45500000,                //record different mode's pclk
        .linelength = 1040,                //record different mode's linelength
        .framelength = 1456,            //record different mode's framelength
...
{	
	...
	write_cmos_sensor(0xfe, 0x00);						  
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xf7, 0x01);//0x01
	write_cmos_sensor(0xf8, 0x1b);//0x0c
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x08,0xE8);//10
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
			LOG_INF("yujun Gc2375 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {
                hardwareinfo_set_prop(HARDWARE_BACK_CAM_ID, "chengxiangtong");
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_GET_CROP_INFO:
            ...
            switch (*feature_data_32) {
                case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
                    //memcpy((char *)wininfo,(char *)&imgsensor_winsize_info[1],sizeof(SENSOR_WINSIZE_INFO_STRUCT));
		    if (copy_to_user((void __user *)wininfo, (void *)&imgsensor_winsize_info[1], sizeof(SENSOR_WINSIZE_INFO_STRUCT))) {
		        LOG_INF("[CAMERA_HW]ERROR: copy_to_user fail\n");
		    }
                    break;
                case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
                    //memcpy((char *)wininfo,(char *)&imgsensor_winsize_info[2],sizeof(SENSOR_WINSIZE_INFO_STRUCT));
		    if (copy_to_user((void __user *)wininfo, (void *)&imgsensor_winsize_info[2], sizeof(SENSOR_WINSIZE_INFO_STRUCT))) {
		        LOG_INF("[CAMERA_HW]ERROR: copy_to_user fail\n");
		    }
                    break;
                case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
                    //memcpy((char *)wininfo,(char *)&imgsensor_winsize_info[3],sizeof(SENSOR_WINSIZE_INFO_STRUCT));
		    if (copy_to_user((void __user *)wininfo, (void *)&imgsensor_winsize_info[3], sizeof(SENSOR_WINSIZE_INFO_STRUCT))) {
		        LOG_INF("[CAMERA_HW]ERROR: copy_to_user fail\n");
		    }
                    break;
                case MSDK_SCENARIO_ID_SLIM_VIDEO:
                    //memcpy((char *)wininfo,(char *)&imgsensor_winsize_info[4],sizeof(SENSOR_WINSIZE_INFO_STRUCT));
		    if (copy_to_user((void __user *)wininfo, (void *)&imgsensor_winsize_info[4], sizeof(SENSOR_WINSIZE_INFO_STRUCT))) {
		        LOG_INF("[CAMERA_HW]ERROR: copy_to_user fail\n");
		    }
                    break;
                case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
                default:
                    //memcpy((char *)wininfo,(char *)&imgsensor_winsize_info[0],sizeof(SENSOR_WINSIZE_INFO_STRUCT));
		    if (copy_to_user((void __user *)wininfo, (void *)&imgsensor_winsize_info[0], sizeof(SENSOR_WINSIZE_INFO_STRUCT))) {
		        LOG_INF("[CAMERA_HW]ERROR: copy_to_user fail\n");
		    }
                    break;
            }
...
UINT32 GC2375HMIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
