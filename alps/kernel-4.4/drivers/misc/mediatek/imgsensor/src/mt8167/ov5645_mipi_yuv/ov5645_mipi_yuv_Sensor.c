...
static void OV5645MIPISetHVMirror(kal_uint8 Mirror, kal_uint8 Mode)
{
	...
	OV5645MIPISENSORDB("[OV5645MIPI]enter OV5645MIPISetHVMirror %d %d:\n ", Mode, Mirror);
...
{
	OV5645MIPISENSORDB
	    ("[OV5645MIPI]enter OV5645_FOCUS_Get_AE_Max_Num_Metering_Areas function:\n ");
	*pFeatureReturnPara32 = 1;
	OV5645MIPISENSORDB(" *pFeatureReturnPara32 = %d\n", *pFeatureReturnPara32);
	OV5645MIPISENSORDB
	    ("[OV5645MIPI]exit OV5645_FOCUS_Get_AE_Max_Num_Metering_Areas function:\n ");
}
...
static void OV5645_FOCUS_OVT_AFC_Get_AF_Status(UINT32 *pFeatureReturnPara32)
{
	...
	*pFeatureReturnPara32 = SENSOR_AF_IDLE;
	...
	mDELAY(1);
	OV5645MIPISENSORDB
	    ("[OV5645MIPI]enter OV5645_FOCUS_OVT_AFC_Get_AF_Status function:state_3028=%d,state_3029=%d\n",
	...
	OV5645MIPISENSORDB
	    ("[OV5645MIPI]exit OV5645_FOCUS_OVT_AFC_Get_AF_Status function:state_3028=%d,state_3029=%d\n",
	     state_3028, state_3029);
}
...
UINT32 OV5645MIPIOpen(void)
{
	...
	for (i = 0; i < 3; i++) {
		...
		OV5645MIPISENSORDB("OV5645MIPI READ ID :%x", sensor_id);
		if (sensor_id != OV5645MIPI_SENSOR_ID) {
			OV5645MIPISENSORDB("[OV5645MIPI] Read Sensor ID Fail[open] = 0x%x\n",
					   sensor_id);
			return ERROR_SENSOR_CONNECT_FAIL;
		}
	}
			 ...
			 MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	OV5645MIPISENSORDB("----Aaron------read register 0x3501=[%x]\n",
			   OV5645MIPIYUV_read_cmos_sensor(0x3501));
	OV5645MIPISENSORDB("----Aaron------read register 0x3502=[%x]\n",
			   OV5645MIPIYUV_read_cmos_sensor(0x3502));
	OV5645MIPISENSORDB("----Aaron------read register 0x350c=[%x]\n",
			   OV5645MIPIYUV_read_cmos_sensor(0x350c));
	OV5645MIPISENSORDB("----Aaron------read register 0x350d=[%x]\n",
			   OV5645MIPIYUV_read_cmos_sensor(0x350d));
	OV5645MIPISENSORDB("----Aaron------read register 0x350b=[%x]\n",
...
BOOL OV5645MIPI_set_param_wb(UINT16 para)
{
	...
	spin_unlock(&ov5645mipi_drv_lock);
	OV5645MIPISENSORDB("[OV5645MIPI] OV5645MIPI_set_param_wb %d\n", para);
...
UINT32 OV5645MIPIYUVSetVideoMode(UINT16 u2FrameRate)
{
	...
	if (u2FrameRate == 30) {
		...
		/* 56Mhz, 224Mbps/Lane, 2Lane. */
		OV5645MIPISENSORDB
		    ("[OV5645MIPI]OV5645MIPIYUVSetVideoMode enter u2FrameRate == 30 setting  :\n ");
		...
		OV5645MIPISENSORDB
		    ("[OV5645MIPI]OV5645MIPIYUVSetVideoMode exit u2FrameRate == 30 setting  :\n ");
	} else if (u2FrameRate == 15) {
		/* ;OV5645MIPI 1280x960,15fps */
		/* 28Mhz, 112Mbps/Lane, 2Lane. */
		OV5645MIPISENSORDB
		    ("[OV5645MIPI]OV5645MIPIYUVSetVideoMode enter u2FrameRate == 15 setting  :\n ");
		...
		OV5645MIPI_write_cmos_sensor(0x350d, 0x00);	/*  */
		OV5645MIPISENSORDB
		    ("[OV5645MIPI]OV5645MIPIYUVSetVideoMode exit u2FrameRate == 15 setting  :\n ");
	} else {
		OV5645MIPISENSORDB("Wrong frame rate setting\n");
	}
...
{
	OV5645MIPISENSORDB
	    ("[OV5645MIPI]enter OV5645MIPIGetDefaultFramerateByScenario function:\n ");
	switch (scenarioId) {
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
		*pframeRate = 300;
...
void OV5645_MIPI_GetExifInfo(UINT32 exifAddr)
{
	...
	OV5645MIPISENSORDB("[OV5645MIPI]exit OV5645_MIPI_GetExifInfo function:\n ");
}
...
static void OV5645MIPI_FlashTriggerCheck(unsigned int *pFeatureReturnPara32)
{
	...
	OV5645MIPISENSORDB("################NormBr is %d\n", NormBr);
