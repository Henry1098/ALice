...
#if 0//def MTK_MULTI_BAT_PROFILE_SUPPORT //Bug-343194,sunxuebin.wt,MODIFY,20180305, merge from wt.guojunbo add second battery data 
...
#include <linux/hardware_info.h>	//Bug-343194,sunxuebin.wt,ADD,20180305,for factory battery id read.
...
//Bug-343194,sunxuebin.wt,MODIFY,20180305,for factory battery id read.
...
//+Bug-343194,sunxuebin.wt,ADD,20180301,for multi battery support.
	if (g_fg_battery_id == 0){		//ATL
		printk("[Battery ID] atl.\n");
		hardwareinfo_set_prop(HARDWARE_BATTERY_INFO, "ATL");
	}else if (g_fg_battery_id == 1){	//SCUD
		printk("[Battery ID] scud.\n");
		hardwareinfo_set_prop(HARDWARE_BATTERY_INFO, "SCUD");
	}else{					//unknown.
		printk("[Battery ID] unknown.\n");
		hardwareinfo_set_prop(HARDWARE_BATTERY_INFO, "UNKNOWN");
	}
//-Bug-343194,sunxuebin.wt,ADD,20180301,for multi battery support.
...
//+Bug-343194,sunxuebin.wt,ADD,20180305, merge from wt.guojunbo add second battery data
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
	if(1==g_fg_battery_id)
	{
		battery_log(BAT_LOG_CRTI, "g_fg_battery_id is 1\n");
		__batt_meter_parse_node(np, "batt_temperature_table_num", &num);

		idx = 0;
		while (!of_property_read_u32_index(np, "batt_temperature_table_1", idx, &addr)) {
			idx++;
			if (!of_property_read_u32_index(np, "batt_temperature_table_1", idx, &val)) {
		...
		__batt_meter_parse_node(np, "battery_profile_t0_num_1", &num);

		__batt_meter_parse_table(np, "battery_profile_t0_1",
			fgauge_get_profile(batt_meter_cust_data.temperature_t0), num);

		__batt_meter_parse_node(np, "battery_profile_t1_num_1", &num);

		__batt_meter_parse_table(np, "battery_profile_t1_1",
			fgauge_get_profile(batt_meter_cust_data.temperature_t1), num);

		__batt_meter_parse_node(np, "battery_profile_t2_num_1", &num);

		__batt_meter_parse_table(np, "battery_profile_t2_1",
			fgauge_get_profile(batt_meter_cust_data.temperature_t2), num);

		__batt_meter_parse_node(np, "battery_profile_t3_num_1", &num);

		__batt_meter_parse_table(np, "battery_profile_t3_1",
			fgauge_get_profile(batt_meter_cust_data.temperature_t3), num);

		__batt_meter_parse_node(np, "r_profile_t0_num_1",	&num);

		__batt_meter_parse_table(np, "r_profile_t0_1",
			(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t0), num);

		__batt_meter_parse_node(np, "r_profile_t1_num_1",	&num);

		__batt_meter_parse_table(np, "r_profile_t1_1",
			(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t1), num);

		__batt_meter_parse_node(np, "r_profile_t2_num_1",	&num);

		__batt_meter_parse_table(np, "r_profile_t2_1",
			(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t2), num);

		__batt_meter_parse_node(np, "r_profile_t3_num_1",	&num);

		__batt_meter_parse_table(np, "r_profile_t3_1",
			(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t3), num);

		__batt_meter_parse_node(np, "q_max_pos_50_1",
			&batt_meter_cust_data.q_max_pos_50);

		__batt_meter_parse_node(np, "q_max_pos_25_1",
			&batt_meter_cust_data.q_max_pos_25);

		__batt_meter_parse_node(np, "q_max_pos_0_1",
			&batt_meter_cust_data.q_max_pos_0);

		__batt_meter_parse_node(np, "q_max_neg_10_1",
			&batt_meter_cust_data.q_max_neg_10);

		__batt_meter_parse_node(np, "q_max_pos_50_h_current_1",
			&batt_meter_cust_data.q_max_pos_50_h_current);

		__batt_meter_parse_node(np, "q_max_pos_25_h_current_1",
			&batt_meter_cust_data.q_max_pos_25_h_current);

		__batt_meter_parse_node(np, "q_max_pos_0_h_current_1",
...
int force_get_tbat(kal_bool update)
{
	...
	return bat_temperature_val - 1;	//ExtB-AQUA-813,sunxuebin.wt,MODIFY,20180411,for bat temp.
#endif
}
...
//+Bug-340320,sunxuebin.wt,DELETE,20180308,for adb read right battery current.
//		if (oam_i_2 < 0)
//			val = -oam_i_2;
//		else
//-Bug-340320,sunxuebin.wt,DELETE,20180308,for adb read right battery current.
