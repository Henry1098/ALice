...
//+bug-351900,guojunbo.wt,ADD,20180427, for safe charger
#define SOC_CHANGE_CV_PONIT 85
kal_bool cv_change_flage=KAL_FALSE;
#define VOLTAGE_CHANGE_CV_PONIT 3600 //+ExtB-AQUA-5515 baidabin.wt,MODIFY,20180714 Add Voltage control Stage Charing Current 
//-bug-351900,guojunbo.wt,ADD,20180427, for safe charger
 /* ============================================================ // */
 /* global variable */
 /* ============================================================ // */
...
//+Bug-337272,sunxuebin.wt,ADD,20180131,for 98995 bringup and charging&battery status controls
#ifdef WT_LOW_TEMP_AND_HIGH_TEMP_CHARGING_CONTROL
unsigned int g_recharging_voltage;      /* sunxuebin.wt add for charging control when temperature in higher or lower range */
#endif
//-Bug-337272,sunxuebin.wt,ADD,20180131,for 98995 bringup and charging&battery status controls
...
#endif
/*Other_platform_modify  sunxuebin.wt  ADD 20161027  ui_soc sync full in charging*/
...
//+Bug-337272,sunxuebin.wt,ADD,20180131,for 98995 bringup and charging&battery status controls
#ifdef WT_LOW_TEMP_AND_HIGH_TEMP_CHARGING_CONTROL
		/* sunxuebin.wt add for charging control when temperature in higher or lower range */
		g_recharging_voltage = batt_cust_data.recharging_voltage;
#endif
//-Bug-337272,sunxuebin.wt,ADD,20180131,for 98995 bringup and charging&battery status controls
	}
}
...
//+Bug-337272,sunxuebin.wt,ADD,20180131,for 98995 bringup and charging&battery status controls
void set_bat_charging_current_limit_temp_control(int current_limit)
{
	battery_log(BAT_LOG_CRTI, "[BATTERY] set_bat_charging_current_limit_temp_control (%d)\r\n",
		    current_limit);
	...
	if (current_limit != -1) {
		...
		if (BMT_status.charger_type == STANDARD_HOST)
			if ((current_limit * 100) >= CHARGE_CURRENT_500_00_MA) {
				...
				battery_log(BAT_LOG_CRTI,
		"[BATTERY] set_bat_charging_current_limit_temp_control over usb spec(%d,%d)\r\n",
...
//+ExtB-AQUA-5515 baidabin.wt,MODIFY,20180714 Add Voltage control Stage Charing Current 
                        if((cv_change_flage==KAL_TRUE)&&(g_temp_CC_value>CHARGE_CURRENT_500_00_MA)&&(BMT_status.bat_vol > VOLTAGE_CHANGE_CV_PONIT)){
...
#ifdef WT_LOW_TEMP_AND_HIGH_TEMP_CHARGING_CONTROL
void low_temp_charging_control(void)
{
	/* if temperature smaller than 15 set charging current to 450mA */
	if (BMT_status.temperature < WT_LOW_TEMP) {
		set_bat_charging_current_limit_temp_control(WT_LOW_TEMP_CHARGING_CURRENT);
	}else {
		set_bat_charging_current_limit_temp_control(-1);
	}
}

void high_temp_charging_control(void)
{
	/* if temperature bigger than 45 set topoff threshold to 4110mV and recharge vol to 4010mV */
	if (BMT_status.temperature >= WT_HIGH_TEMP) {
		v_cc2topoff_threshold = WT_HIGH_TEMP_TOPOFF_THRESHOLD;
		g_recharging_voltage = WT_HIGH_TEMP_RECHARGING_VOLTAGE;
	}else {
		v_cc2topoff_threshold = batt_cust_data.v_cc2topoff_thres;
		g_recharging_voltage = batt_cust_data.recharging_voltage;
	}
...
/*Other_platform_modify  sunxuebin.wt ADD 20161028  ui_soc sync full in charging*/
#ifdef WT_UI_SOC_SYNC_FULL_IN_CHARGING
...
//+Bug-337272,sunxuebin.wt,ADD,20180131,for 98995 bringup and charging&battery status controls
#ifdef WT_LOW_TEMP_AND_HIGH_TEMP_CHARGING_CONTROL
				/* sunxuebin.wt add for charging control when temperature in lower range */
				low_temp_charging_control();
#endif
//-Bug-337272,sunxuebin.wt,ADD,20180131,for 98995 bringup and charging&battery status controls
			}
		}
...
#if !defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
			if (batt_cust_data.high_battery_voltage_support) {
				cv_voltage = BATTERY_VOLT_04_250000_V;	
                                if(cv_change_flage==KAL_TRUE){   //bug-351900,guojunbo.wt,ADD,20180427, for safe charger
                                 cv_voltage = BATTERY_VOLT_04_350000_V;
                                }
#ifdef WT_LOW_TEMP_AND_HIGH_TEMP_CHARGING_CONTROL
				/* sunxuebin.wt modified for charging control when temperature in higher or lower range */
				if (BMT_status.temperature >= WT_HIGH_TEMP) 
					cv_voltage = WT_HIGH_TEMP_CV_VOLTAGE;
#endif
			}
			else
				cv_voltage = BATTERY_VOLT_04_200000_V;
...
PMU_STATUS BAT_TopOffModeAction(void)
{
	...
	unsigned int cv_voltage;
//+Bug-337272,sunxuebin.wt,MODIFY,20180131,for 98995 bringup and charging&battery status controls
	if (batt_cust_data.high_battery_voltage_support) {
		cv_voltage = 4400;	//ExtB-AQUA-394,sunxuebin.wt,MODIFY,20180330,for 4.4V HV battery can charging full.
#ifdef WT_LOW_TEMP_AND_HIGH_TEMP_CHARGING_CONTROL
		/* sunxuebin.wt modified for charging control when temperature in higher or lower range */
		if (BMT_status.temperature >= WT_HIGH_TEMP) 
			cv_voltage = WT_HIGH_TEMP_CV_VOLTAGE;
...
//+Bug-337272,sunxuebin.wt,MODIFY,20180131,for 98995 bringup and charging&battery status controls
#ifdef WT_LOW_TEMP_AND_HIGH_TEMP_CHARGING_CONTROL
if (BMT_status.bat_vol < g_recharging_voltage) /* sunxuebin.wt modified for charging control when temperature in higher or lower range */
#else
...
PMU_STATUS BAT_BatteryStatusFailAction(void)
{
	...
	/* sunxuebin.wt modified for Factory test*/
	if (BMT_status.UI_SOC != 100)
	{
		battery_log(BAT_LOG_CRTI, "Clear the battery full flags !!\n\r");
...
#endif
        //+bug-351900,guojunbo.wt,ADD,20180427, for safe charger
        if(BMT_status.UI_SOC>=SOC_CHANGE_CV_PONIT){
           cv_change_flage=KAL_TRUE;
        }else{
           cv_change_flage=KAL_FALSE;
        }
       //-bug-351900,guojunbo.wt,ADD,20180427, for safe charger
