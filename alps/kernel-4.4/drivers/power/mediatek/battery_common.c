...
kal_bool g_runin_status = KAL_FALSE;	//Bug-336692,sunxuebin.wt,ADD,20180125,for runin test soc is 100 but power off
//-Bug-337272,sunxuebin.wt,ADD,20180131,for 98995 bringup and charging&battery status controls
static int nonstand_cnt = 0;   //ExtB-AQUA-1356,zhaosidong.wt,MODIFY,20180426,for charger type detection.
...
//+Bug356894,zhaosidong.wt,ADD,20180531,add procfs check battery volumes
static int proc_read_capacity_show(struct seq_file *m, void *v)
{
    int capacity = BMT_status.UI_SOC;
    seq_printf(m, "%d\n", capacity);
    return 0;
}
...
int init_proc_log(void)
{
	...
	battery_log(BAT_LOG_CRTI, "proc_create bat_capacity_proc_fops\n");
	return ret;
}
//-Bug356894,zhaosidong.wt,ADD,20180531,add procfs check battery volumes
					 ...
					 char *buf)
{
	...
	ret_value = batt_cust_data.v_cc2topoff_thres;	//Bug-337272,sunxuebin.wt,ADD,20180131,for 98995 bringup and charging&battery status controls
...
//Other sunxuebin.wt add 20180125 start charging/stop charging
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if (resetBatteryMeter == KAL_TRUE) {
		...
		if (BMT_status.bat_full == KAL_TRUE && is_uisoc_ever_100 == KAL_TRUE && BMT_status.ruin_stop != KAL_TRUE) { //wt.sunxuebin merged
			BMT_status.UI_SOC = 100;
...
CHARGER_TYPE mt_charger_type_detection(void)
{
	...
	if ((BMT_status.charger_type == CHARGER_UNKNOWN) ||
		(BMT_status.charger_type == NONSTANDARD_CHARGER)) {    //ExtB-AQUA-1356,zhaosidong.wt,MODIFY,20180426,for charger type detection.
#else
	if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
...
#else
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
//+ExtB-AQUA-1356,zhaosidong.wt,MODIFY,20180426,for charger type detection.
		if ((BMT_status.charger_type == CHARGER_UNKNOWN) ||
...
//+Bug-340827,sunxuebin.wt,MODIFY,20180308,for PowerOff Charging icons merge from zhaoweihuan.
/*
...
//+Bug-340827,sunxuebin.wt,ADD,20180308,for PowerOff Charging icons merge from zhaoweihuan.
static struct wake_lock chr_lock;
static void mt_charger_lock(void)
{
	if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
		|| g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT){
		wake_lock_init(&chr_lock, WAKE_LOCK_SUSPEND, "charger wakelock");
		wake_lock(&chr_lock);
	}
}
//-Bug-340827,sunxuebin.wt,ADD,20180308,for PowerOff Charging icons merge from zhaoweihuan.
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		/*Other sunxuebin.wt add 20180125 for runin start*/
	...
	BMT_status.charger_exist = KAL_FALSE;	/* for default, no charger */
        BMT_status.ruin_stop = KAL_FALSE; //wt.sunxuebin mergerd
