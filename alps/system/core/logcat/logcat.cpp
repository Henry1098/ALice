...
//+ huangyabin.wt,add,20180305,add for wtlog
#ifdef USE_WT_LOG
#include <pthread.h>
...
#define DEFAULT_MAX_COMPRESS_LOGS 10
#define WRITEBUFFERSIZE (8192)

#define NUM_ROTATE_FILES (5)
#define LEN_OF_TEMP_BUFFER (409600)
#define PATH_OF_CACHE_LOGCAT_LOG "/cache/wt_logs/logcat-log"
#endif
//- huangyabin.wt,add,20180305,add for wtlog
...
struct android_logcat_context_internal {
    ...
    #ifdef USE_WT_LOG
    //0 means "no compress"
    size_t maxComPressLogs = DEFAULT_MAX_COMPRESS_LOGS;
    char outputDirname[PATH_MAX] ={0};
    char outputDirnameTmp[PATH_MAX] ={0};
    char outputCompressDirname[PATH_MAX] ={0};
    #endif
    //-huangyabin.wt,add,20180305,add for wtlog
...
//+huangyabin.wt,add,20180305,add for wtlog
#ifdef USE_WT_LOG
int getdirname(const char *file_name, char *dir_name)
{
    ...
    strncpy(file_path, file_name, strlen(file_name));
    for (i=len; i >0; i--) {
        if (file_path[i] == '/') {
            file_path[i] = '\0';
            strncpy(dir_name, file_path, strlen(file_path));
            break;
        }
    }
    if(i == 0) return -1;
...
int find_files(char *prefix, char *path,  char* suffix, android_logcat_context_internal* context)
{
    struct dirent * ptr;
    DIR * dir;
    ...
    dir = opendir(context->outputDirname);
    if(dir == NULL) {
        printf("opendir log failed.\n");
        return -1;
    }
    while((ptr = readdir(dir)) != NULL)
    {
        ...
        //printf("%s current files %s \n", __func__, ptr->d_name);
        memset(temp, 0, sizeof(temp));
        strncpy(temp, ptr->d_name, strlen(ptr->d_name));
        temp_suffix=strstr(temp, "_");
        if (!strncmp(prefix, temp, strlen(prefix))) {
            strncpy(suffix, temp_suffix, strlen(temp_suffix));
            strncpy(path, temp, strlen(temp));
            //printf("%s find files: %s \n", __func__,suffix);
            closedir(dir);
            return 0;
        }
    }
...
static void *pthread_Compresslogs(void* args)
{
    char *command_string, *outputDirName;
    char * outputfilename;
    char *outputCompressDirname;
    char *filesToCompress[MAX_FILE_NUM];
    char tar_dirname[PATH_MAX] = { 0 };
    int err = -1, ret = -1, i = 0;

    android_logcat_context_internal* context = (android_logcat_context_internal*)args;
    for (i = context->maxComPressLogs ; i > 0 ; i--) {
        char *file0_prefix, *file1_prefix;
        char file0_find[PATH_MAX] = {0}, file1_replaced[PATH_MAX] = {0};
        char file0_suffix[PATH_MAX] = {0}, file0_path[PATH_MAX] ={0};
        char file0_prefix_temp[PATH_MAX] ={0}, file1_prefix_temp[PATH_MAX] ={0};

        memset(file0_find, 0, sizeof(file0_find));
        memset(file1_replaced, 0, sizeof(file1_replaced));
        memset(file0_suffix, 0, sizeof(file0_suffix));
        memset(file0_path, 0, sizeof(file0_path));
        memset(file0_prefix_temp, 0, sizeof(file0_prefix_temp));
        memset(file1_prefix_temp, 0, sizeof(file1_prefix_temp));

        sprintf(file1_prefix_temp, "%s-zip.%d", context->outputFileName,i);
        sprintf(file0_prefix_temp, "%s-zip.%d", context->outputFileName,i-1);
        file0_prefix = strrchr(file0_prefix_temp, '/');
        file0_prefix++;
        file1_prefix = strrchr(file1_prefix_temp, '/');
        file1_prefix++;

        //printf("file0_prefix: %s, file1_prefix: %s \n", file0_prefix,file1_prefix);
        if((size_t)i == context->maxComPressLogs) {
            ret = find_files(file1_prefix, file0_find, file0_suffix, context);
            sprintf(file0_path, "%s/%s", context->outputDirname,file0_find);
            //printf("i = g_maxComPressLogs .ret = %d, file0_path:%s \n", ret, file0_path);
            if(ret == 0) {
                printf("find max files:%s ,will delete it .\n", file0_path);
                unlink(file0_path);
            }
            memset(file0_find, 0, sizeof(file0_find));
            memset(file0_suffix, 0, sizeof(file0_suffix));
        ...
        ret = find_files(file0_prefix, file0_find, file0_suffix, context);
        //printf("ret = %d, file0_path:%s \n", ret, file0_find);
        if (ret == 0) {
            sprintf(file1_replaced, "%s/%s%s", context->outputDirname,file1_prefix,file0_suffix);
            sprintf(file0_path, "%s/%s", context->outputDirname,file0_find);
            //printf("file0_path: %s, file1_replaced: %s \n", file0_path,file1_replaced);
            err = rename(file0_path, file1_replaced);
            //printf("err= %d, errno: %s \n", err,strerror(errno));
            if (err < 0 && errno != ENOENT) {
                fprintf (stderr, "while move log files from %s to %s failed", file0_path, file1_replaced);
            }
        }
    }

    outputCompressDirname = context->outputDirnameTmp;
    outputfilename = (char*)strrchr(context->outputFileName, '/');
    outputfilename++;

    int maxRotationCountDigits =
            (context->maxRotatedLogs > 0) ? (int) (floor(log10(context->maxRotatedLogs) + 1)) : 0;

    for (unsigned int i=0; i < context->maxRotatedLogs; i++) {
        asprintf(&filesToCompress[i], "%s/%s.%.*d", outputCompressDirname,outputfilename, maxRotationCountDigits, i+1);
    }
    ...
    p = gmtime(&timep);
    char *cur_time;
    asprintf(&cur_time, "%d_%d_%d_%d_%d", (p->tm_year+1900),(1+p->tm_mon),p->tm_mday, p->tm_hour,p->tm_min);
    //bug280675,huangrongbiao.wt,modify,20170725,modify to tar.gz
    asprintf(&command_string, "%s-zip.0_%s.tar.gz", context->outputFileName,cur_time);
    free(cur_time);
    wt_compress_zip_files(context->maxRotatedLogs, (const char **)filesToCompress ,command_string,  0);

    outputfilename = (char*)strrchr(context->outputFileName, '/');
    outputfilename++;

    for (int i = context->maxComPressLogs ; i >= 0 ; i--) {
        if (i == 0) {
            asprintf(&command_string, "%s/%s", outputCompressDirname,outputfilename);
        }
        asprintf(&command_string, "%s/%s.%.*d", outputCompressDirname,outputfilename,maxRotationCountDigits, i);
        unlink(command_string);
    }
    rmdir(outputCompressDirname);

    free(command_string);
    for (i=0; (size_t)i < context->maxRotatedLogs; i++) {
...
static void CompressLogs(android_logcat_context_internal* context)
{
    char *outputDirName, *outputfilename;
    pthread_t pid;
    int err = -1;

    //fprintf (stderr, "%s , Entering..... \n", __func__);
    asprintf(&outputDirName, "%s-tmp", context->outputFileName);
    delete_file(outputDirName);
    if(mkdir(outputDirName, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH) == -1) {
        fprintf (stderr, "%s mkdir %s failed \n", __func__,outputDirName);
        free(outputDirName);
        return;
    }
    outputfilename = (char*)strrchr(context->outputFileName, '/');
    ...
    for (int i = context->maxComPressLogs ; i > 0 ; i--) {
        ...
        asprintf(&file1, "%s%s.%.*d", outputDirName,outputfilename, maxRotationCountDigits, i);
        asprintf(&file0, "%s.%.*d", context->outputFileName, maxRotationCountDigits, i);

        //fprintf (stderr, "%s move %s to %s..................................\n", __func__,file0, file1);
        err = rename (file0, file1);
        if (err < 0 && errno != ENOENT) {
            fprintf (stderr, "while move log files from %s to %s failed", file0, file1);
        }
        free(file0);
        free(file1);
    }

    strcpy(context->outputDirnameTmp, outputDirName);

    pid = pthread_create(&pid, NULL, pthread_Compresslogs, context);
    if (pid != 0) {
        fprintf (stderr, "%s:pthread_create failed for compress logs.\n", __func__);
        exit(-1);
    }
}
#endif
//-req,bug335109,huangyabin.wt,add,20180122,add for wtlog
...
static void rotateLogs(android_logcat_context_internal* context) {
    ...
    #ifdef USE_WT_LOG
    char *file_rotate;
    asprintf(&file_rotate, "%s.%.*d", context->outputFileName,maxRotationCountDigits,(int)context->maxRotatedLogs);
    if( access(file_rotate, R_OK) == 0 && context->maxComPressLogs) {
        CompressLogs(context);
        //fprintf (stderr, "%s, compresslogs() end.....\n", __func__);
    }
    free(file_rotate);
    #endif
    //-huangyabin.wt,add,20180305,add for wtlog
...
//+huangyabin.wt,add,20180305,add for wtlog
/* begin,catch kmsg logcat log before data encrypted. */
#ifdef USE_WT_LOG
static void wtRotateLogs(const char* pathname)
{
    int err;
    int i = 0;
    char *file_rotate;
    ...
    for (i = NUM_ROTATE_FILES ; i > 0 ; i--) {
        char *file0, *file1;
        asprintf(&file1, "%s.%d", pathname, i);
        if (i - 1 == 0) {
            asprintf(&file0, "%s", pathname);
        } else {
            asprintf(&file0, "%s.%d", pathname, i - 1);
        }
...
//event log:  event-log
static void readCacheWtlogs(android_logcat_context_internal* context)
{
    char buf[LEN_OF_TEMP_BUFFER] = {0};
    char buf_path[PATH_MAX]= {0};
    int fd = 0;
    int n = 0;
    ...
    if(strcmp(context->outputFileName, PATH_OF_CACHE_LOGCAT_LOG) != 0) {
        if((access(PATH_OF_CACHE_LOGCAT_LOG, R_OK) == 0) && strstr(context->outputFileName, "logcat-log") != 0) {
            fd = open(PATH_OF_CACHE_LOGCAT_LOG ,O_RDONLY);
            if(fd > 0) {
                n = read(fd, buf, sizeof(buf)-1);
                if(n > 0) {
                    write(context->output_fd, buf, strlen(buf));
                }
                close(fd);
            }
            wtRotateLogs(PATH_OF_CACHE_LOGCAT_LOG);
        }
    }
}
#endif
/* end,catch kmsg logcat log before data encrypted. */
//-bug335109,huangyabin.wt,add,20180122,add for wtlog
...
static int __logcat(android_logcat_context_internal* context) {
    ...
    for (;;) {
        ...
        //+huangyabin.wt,add,20180305,add for wtlog
        #ifdef USE_WT_LOG
        ret = getopt_long_r(argc, argv, ":cdDhLt:T:gG:sQf:r:z:n:v:b:BSpP:m:e:",
        ...
        switch (ret) {
            ...
            case 'f':
                ...
                //+huangyabin.wt,add,20180305,add for wtlog
                #ifdef USE_WT_LOG
                ret= getdirname(context->outputFileName, context->outputDirname);
                printf("outputFileName is %s, outputDirname is %s. \n", context->outputFileName, context->outputDirname);
                if(ret == -1) {
                    printf("get output directory failed. \n");
                    getdirname(context->outputFileName, context->outputDirname);
                }
                #endif
                //-huangyabin.wt,add,20180305,add for wtlog
            ...
            #ifdef USE_WT_LOG
            case 'z':
                if (!getSizeTArg(optctx.optarg, &context->maxComPressLogs, 1)) {
                    logcat_panic(context, HELP_TRUE,
                                 "Invalid parameter \"%s\" to -r\n",
                                 optctx.optarg);
                }
