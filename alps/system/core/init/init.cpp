...
static char secboot_fuse[8];
static char secureboot_reg[92]="/proc/secboot_fuse_reg";
namespace android {
namespace init {
...
static void import_kernel_nv(const std::string& key, const std::string& value, bool for_emulator) {
    ...
    } else if(key == "recovery_reason" ){
        if(value == "by_rtc")
			property_set("ro.recovery.reason.rtc", "1" );	
	}
...
static void export_kernel_boot_props() {
    int fd=open(secureboot_reg,O_RDWR | O_CLOEXEC);
    if (fd>0)
       read(fd,secboot_fuse,sizeof(secboot_fuse));
    close(fd);
    struct {
        const char *src_prop;
        const char *dst_prop;
	...
	{ "ro.boot.pcbaserialno",   "ro.pcbaserialno",   "", },//bug 345195 add ro.pcbaserialno 0~23  PSN
        { "ro.boot.pcbaserialno",   "gsm.sn",   "unknown", },//bug 356886,zhaoweihuan.wt, add, 20180528,add gsm.sn for PSN
        { "ro.boot.serialno",   "sys.lenovosn",   "unknown", }, //bug 356887,zhaoweihuan.wt, add, 20180528,add sys.lenovosn for GSN
    };
    for (size_t i = 0; i < arraysize(prop_map); i++) {
        std::string value = GetProperty(prop_map[i].src_prop, "");
        property_set(prop_map[i].dst_prop, (!value.empty()) ? value : prop_map[i].default_value);
    }
    if (strncmp(secboot_fuse,"0x303030",8)==0)
       property_set("ro.boot.secureboot","1");
    else
       property_set("ro.boot.secureboot","0");
}
