...
//bug 340347 yuanjian.wt , 20180306, first recovery
static const char *LAST_RESET_RECOVERY_FILE = "/cache/recovery/last_reset_recovery";
//Bug340350,shenhaojie.wt,2018.03.06,add for factoryrest
static const char *FACTORY_FLAG = "/cache/recovery/last_wipedata";
static const char *FACTORY_FLAG_VS = "/cache/vs_flag";
bool first_recovery_flag = false;
...
static bool wipe_data(Device* device) {
    ...
    //Bug340350,shenhaojie.wt,2018.03.06,add for factoryrest -- start
    if(success){
        FILE* fp3 = fopen(FACTORY_FLAG, "w");
        fclose(fp3);
        FILE* fp4 = fopen(FACTORY_FLAG_VS, "w");
        fclose(fp4);
    }
    //Bug340350,shenhaojie.wt,2018.03.06,add for factoryrest -- end
    return success;
}
...
//bug 340347 yuanjian.wt , 20180306, first recovery
int set_first_reset_state() 
{

	FILE* fp = fopen_path(LAST_RESET_RECOVERY_FILE, "w+");
	const char* has_reset = "1";
     if (fp) {
        fwrite(has_reset, 1, 1, fp);
		fsync(fileno(fp));
        fclose(fp);
    }
	return 0;
...
int main(int argc, char **argv) {
    ...
    //bug 340347 yuanjian.wt , 20180306, first recovery
    if(!should_wipe_data  &&  !sideload	&& !shutdown_after &&  !sideload_auto_reboot
	   && !should_wipe_cache &&  !just_exit  &&  !show_text && !security_update)
    {
	   char property_value[PROPERTY_VALUE_MAX] = {0};
	   property_get("ro.recovery.reason.rtc", property_value, "0"); 
	   if(strcmp(property_value,"1") == 0 && (access(LAST_RESET_RECOVERY_FILE,0)  == -1))
	   {
		   should_wipe_data = true;
		   first_recovery_flag = true;
	   }
    }
	//bug 340347 yuanjian.wt , 20180306, first recovery
    ...
    if (update_package != NULL) {
		...
		//bug 340347 yuanjian.wt , 20180306, first recovery
		if(first_recovery_flag == true ){
            set_first_reset_state();  
		}
      ...
      // +bug 371948, liuyanfeng.wt, modify, 20180718, for recovery mode
      //if (is_ro_debuggable()) {
        ui->ShowText(true);
      //}
      // -bug 371948, liuyanfeng.wt, modify, 20180718, for recovery mode
