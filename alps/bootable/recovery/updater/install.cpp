...
// check cache file for power down
int CheckCacheSizeFile(){
    if (0 == access(FILE_PATH_FOR_CACHE_SIZECHECK, F_OK))
    {
        printf("%s: cache size file is exist.\n", __func__);
        return 1;
    }
    else
    {
        printf("%s: cache size file is not exist.\n", __func__);
        return 0;
    }
}
//-bug 346148, huanghaiquan.wt, modify, 20180320, for  ota shutdown
