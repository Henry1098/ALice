...
//+bug 346148, huanghaiquan.wt, modify, 20180320, for  ota shutdown
//create a file for power down
static void CreateCacheSizeFile(){
    int fd = -1;
    fd = open(FILE_PATH_FOR_CACHE_SIZECHECK, O_WRONLY|O_CREAT, 0755);
    if(fd < 0) {
        printf("%s: can't create file for cache size.\n", __func__);
    }
    else
    {
        printf("%s: create file for cache size.\n", __func__);
    }
    close(fd);
}

//delete cache file for power down
void DeleteCacheSizeFile(){
    printf("%s: delete cache size file.\n", __func__);
    unlink(FILE_PATH_FOR_CACHE_SIZECHECK);
}
//-bug 346148, huanghaiquan.wt, modify, 20180320, for  ota shutdown
...
Value* BlockImageUpdateFn(const char* name, State* state,
    ...
    //+bug 346148, huanghaiquan.wt, modify, 20180320, for  ota shutdown
    #if 1
    Value* imageupdate = PerformBlockImageUpdate(name, state, argv, commands,
